...
contract TicTacToe {
    ...
    function _getStatus(uint pos) private view returns (uint) {
        ...
        for (uint i = 0; i <= 2; i++) {
            /*columns*/
            if (_threeInALine(i,3+i,6+i)) {
                status = board[i];
            }
            /*rows*/
            if (_threeInALine(3*i,3*i+1,3*i+2)) {
                status = board[3*i];
            }
        }
        /*diagonals*/
    ...
    function myTurn() public view returns (bool) {
       /*Please complete the code here.*/
       if((msg.sender == players[0] && turn == 1) || (msg.sender == players[1] && turn == 2)){
           return true;
       } else{
           return false;
       }
    ...
    modifier _myTurn() {
      /*Please complete the code here.*/
      require((msg.sender == players[0] && turn == 1) || (msg.sender == players[1] && turn == 2));
      _;
      if (turn==1) {
          turn = 2;
      } 
